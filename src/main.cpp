...
GLuint normalTex = 0;
GLuint normalTexOnGPU = 0;
GLuint albedoTex = 0;
GLuint albedoTexOnGPU = 0;

bool isTexturedColor = false;
bool isTexturedNormal = false;
...
struct Scene {
  ...
  void render()
  {
    ...
    mainShader->set("material.albedoTex", (int) albedoTexOnGPU);

    isTexturedColor = true;
    isTexturedNormal = true;
    // back-wall
    ...
    mainShader->set("isTexturedColor", (int) isTexturedColor);
    mainShader->set("isTexturedNormal", (int) isTexturedNormal);
    plane->render();
    ...
    isTexturedColor = false;
    isTexturedNormal = false;
    // rhino
...
void initScene(const std::string &meshFilename)
{
  ...
  // Load meshes in the scene
  {
    ...
    g_scene.rhino->init(g_cam->getPosition());

    g_scene.plane = std::make_shared<Mesh>();
    g_scene.plane->addPlan();
    g_scene.plane->init(g_cam->getPosition());
